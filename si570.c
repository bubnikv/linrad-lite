...
//        printf("firmware type 1\n");
                    ...
                    if(strcmp((const char *)buffer, VENDOR_NAME_OBDEV ) == 0) {
                        ...
                        (dev_handle,
                         ...
                         buffer,
                         sizeof(buffer));
                        if (strcmp((const char *)buffer, PRODUCT_NAME_DG8SAQ) == 0 || strcmp((const char *)buffer, "Peaberry SDR") == 0) {
                            libusb_get_string_descriptor_ascii
                            (dev_handle,
                             desc.iSerialNumber,
                             buffer,
...
void si570_test_read_buttons_AD(void)
{
    unsigned char value;
    int nBytes;
    if (si570_open_switch == 0)
        ensure_usb_open();
    lir_mutex_lock(MUTEX_LIBUSB);
    nBytes = libusb_control_transfer
             (rx_global_si570usb1_handle,
              LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE | LIBUSB_ENDPOINT_IN,
              0x2e,
              0,
              0,
